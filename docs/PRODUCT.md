...
2. **Loudness fix** behind a top-level **DJ | Streaming** switch. DJ: one fixed, persisted **DJ target** on S-P95 (default -11 until measured; "Calibrate from my library" sets it once) and **Club hot** -8. Streaming: Spotify -14, Apple -16 on Integrated. Targets can be set from -30 to -4 LUFS and ceilings from -6 to 0 dBTP. Gain only, ceiling-capped (-0.5 dBTP DJ, -1.0 streaming), "Short by X LU" when headroom runs out; MP3 via lossless `global_gain` in 1.5 dB steps with the residual shown. Per-track override.
...
7. **Batch UX**: drop folders/files, analyse-only, Needs-review queue (`N`/`Enter`/`Esc`), filter chips, progress and cancel, resume from cache, per-batch CSV. The track list is remembered across restarts (analyses come back from the cache; a file moved or deleted since shows as "File not found"); **Clear list** empties it without touching any file.
8. **`sc-cli`** with `analyze <files>` (text, or `--json` with exactly the table's fields, one document per file; `--no-grid` for loudness only, `--bpm-range 70-180` for the DJ app's range, `--model small|full`, `--evidence` to include the model beats, activations and onsets a refit uses, `--no-cache`, `--jobs N` files at once with reports still printed in the order given), `plan <files>` (what processing would do to each file, decided exactly as the table decides it: `--mode dj|streaming`, `--target`, `--ceiling`; the Action, Status and review reasons per file, then the counts the filter chips show), `bench <file>` (time and real-time factor of each stage), `eval --labels <csv> --dir <folder>` (scores BPM, meter, bar 1 and integrated loudness against hand labels; `scripts/ffmpeg-reference.sh` writes the loudness reference), `cache path|clear` (the analysis cache lives under `~/Library/Caches/app.soundcheck.desktop/analysis`, keyed by file size and time, settings and app version), `process`, `grid-check`, `undo`. A file that fails prints an error document and the exit code is 2; a missing beat-tracking model stops the run before any file with the directories searched.
