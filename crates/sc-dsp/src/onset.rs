...
//! stays flat. The envelope is sampled per 1 ms hop, converted to dB, and its positive first
//! difference is peak-picked: a hop is an onset when its rise is the largest within
//! +/-[`MIN_SPACING_MS`], exceeds `median + k * MAD` of all rises (a robust threshold that
//! ignores the small fluctuations of a sustained band) and exceeds [`MIN_RISE_DB`]. The method
...
impl OnsetDetector {
    ...
    pub fn detect(&self, mono: &[f32]) -> Vec<Onset> {
        ...
        for (k, &rise) in rises.iter().enumerate() {
            // The first window is warm-up: the band-pass ahead of this detector settles over
            // its first milliseconds and would read as an attack.
            if k + 1 < spacing || rise <= threshold || rise <= 0.0 {
                continue;
            }
            let lo = k.saturating_sub(spacing);
            let hi = (k + spacing + 1).min(rises.len());
            ...
            if is_local_max {
                // Rises are dB differences of modest size; f32 keeps every meaningful digit.
                #[allow(clippy::cast_possible_truncation)]
                let rise_db = rise as f32;
                onsets.push(Onset {
                    frame: (k + 1) * self.hop_frames,
                    rise_db,
                });
            }
        }
